...
# Load mid_res_model
with open('data/mid_res_model.pkl', 'rb') as f:
    model = pickle.load(f)
...
plt.title('Actual vs Predicted File Size (Mid-Res Model)')
plt.plot([actual_file_size.min(), actual_file_size.max()], [actual_file_size.min(), actual_file_size.max()], 'k--', lw=2)
plt.savefig('data/plots/mid_res_model_performance.png')
plt.close()
...
plt.title('Relative Prediction Errors by Resolution (Mid-Res Model)')
plt.savefig('data/plots/mid_res_model_errors.png')
plt.close()